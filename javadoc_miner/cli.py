...
from .diff_extractor import commit_has_javadoc_and_code_changes, extract_file_changes
from .git_repo import GitCommandError, GitRepo
from .issue_finder import find_issues
from .java_parser import parse_entities
from .models import Classification, EntityDoc, OutputSample
        ...
        for file_change in file_changes:
            ...
            for old_entity, new_entity, classification in _classify_file_entities(
                ...
                if len(samples) >= config.max_samples:
                    SampleWriter(config.output_dir).write_samples(samples)
...
def _classify_file_entities(
    ...
    min_quality: str,
) -> list[tuple[EntityDoc | None, EntityDoc, Classification]]:
    results: list[tuple[EntityDoc | None, EntityDoc, Classification]] = []
    ...
    for new_index, new_entity in enumerate(new_entities):
        ...
        classification = classify_entity_change(old_entity, new_entity, nearby_code_changed=True)
...
def _build_output_sample(
    ...
    return OutputSample(
        ...
        new_javadoc=new_entity.javadoc,
        patch=patch,
        commit_message=commit_message,
