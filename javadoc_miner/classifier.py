...
def classify_entity_change(
    ...
    if old is None:
        if normalize_doc_text(new.javadoc):
            return Classification(_addition_type(new), "B")
        return None
    ...
    if old.entity_type != new.entity_type:
        return Classification("class_api_change", "A")
    if old.name != new.name:
        return Classification("method_rename" if new.entity_type == "method" else "class_api_change", "A")
    if old.parameters != new.parameters:
        return Classification("parameter_change", "A")
    if old.return_type != new.return_type:
        return Classification("return_type_change", "A")
    if old.throws != new.throws:
        return Classification("exception_change", "A")
    if nearby_code_changed:
        return Classification("nearby_code_and_javadoc_change", "C")
    return None
...
def _addition_type(entity: EntityDoc) -> str:
    return "class_addition" if entity.entity_type == "class" else "method_addition"
